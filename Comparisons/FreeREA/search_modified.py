...
from pruning_modified import prune, prune_latency_params
from nas_utils_modified import *
from genotypes import mutate


def search(api, N=25, n=5, P=[1], R=[1], max_latency=0.1, max_params=0.4, max_time=2, n_random=0, analyzer=None, scaler=None, encoder=None, tokenizer=None):
    ...
    exemplars = population_init(api, N, max_latency, max_params, analyzer=None)
    # Update history
    ...
    exemplars, history = prune(exemplars, history, max_latency=max_latency,
    ...
    while total_time <= max_time:
        ...
        while True:
            ...
            # feasibility pruning
            exemplars, history = prune_latency_params(exemplars, history, max_latency=max_latency,
                                                    max_params=max_params)
            if len(exemplars) > N:
                break

        # ageing and training free metrics pruning
        exemplars, history = prune(exemplars, history, max_latency=max_latency, max_params=max_params,
    ...
    top1_hist = info(api, exemplars, history, start, metrics=metrics, max_latency=max_latency, max_params=max_params, scaler=scaler, encoder=encoder, tokenizer=tokenizer)
