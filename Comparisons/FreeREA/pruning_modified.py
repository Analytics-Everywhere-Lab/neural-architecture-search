...
def prune_latency_params(exemplars, history, max_latency, max_params):
    exemplars, history = dictionary_update(exemplars, history)
    history = clean_history(history, max_params, max_latency)

    # Return only exemplars with latency and params below threshold
    exemplars = [exemplar for exemplar in exemplars if
                 exemplar.get_cost_info()['latency'] <= max_latency
...
def prune(exemplars, history, max_latency=0, max_params=0, K=0, metrics=[], feasibility=True):
    if feasibility:
        exemplars, history = prune_latency_params(exemplars, history, max_latency, max_params)
