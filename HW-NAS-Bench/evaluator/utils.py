# This version of utils allows flexible arguments into the model design to tailor to the desired number of metrics
# It also includes a flexible MakeDataset to decide which metrics are needed
...
class MakeDataset(Dataset):
    def __init__(self, df, tokenizer, scaler, accuracy=True, score=False, memory=False, latency=False):
        self.df = df
        self.tokenizer = tokenizer
        self.scaler = scaler
        self.archs = self.df['Architectures']
        if accuracy == False: # For FBNet
            self.metrics = torch.tensor(self.df['Latency'])
        if score == True:
            self.metrics = torch.tensor(zip(self.df['Score'], self.df['Memory']))    
        if latency == True:
            self.metrics = torch.tensor(zip(self.df['Accuracy'], self.df['Latency']))
        if memory == True:
            self.metrics = torch.tensor(zip(self.df['Accuracy'], self.df['Memory']))
        
        # Check if metric is 1D or 2D and reshape for 1D array
        if self.metrics.ndim == 1:
            self.metrics = self.metrics.reshape(-1, 1)
    ...
    def __getitem__(self, idx):
        arch = self.archs[idx]
        if isinstance(self.scaled_metrics[idx], tuple):    
            acc, metric = self.scaled_metrics[idx]
            metric_tensor = torch.tensor([acc, metric], dtype=torch.float)
        else: # A single-objective evaluations
            metric_tensor = torch.tensor(self.scaled_metrics[idx], dtype=torch.float)
...
class Encoder(nn.Module):
    def __init__(self, model_name='t5-small', num_outputs=2): # Vary the number of outputs according to how many target metrics you want
